...
public class MicroDao extends DaoGenerique<Micro, Long> {
    ...
    private static final RowMapper<Micro> MAPPER = rs -> new Micro(
            rs.getLong("id"),
    ...
    public List<Micro> findByEnregistreur(String idEnregistreur) {
        return query("SELECT * FROM microphone WHERE recorder_id = ? ORDER BY id DESC", MAPPER, idEnregistreur);
    }
    ...
    public Optional<Micro> trouverActifParEnregistreur(String idEnregistreur) {
        return queryUnique("SELECT * FROM microphone WHERE recorder_id = ? AND is_active = 1", MAPPER, idEnregistreur);
    }
