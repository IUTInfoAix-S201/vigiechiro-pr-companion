...
public final class FiltresMultisiteJson {
    ...
    public static FiltresMultisite interpreter(String json) {
        ...
        Verdict verdict = champs.containsKey(CLE_VERDICT) ? Verdict.parLibelle(champs.get(CLE_VERDICT)) : null;
    ...
    private static void sauterEspacesEtVirgules(String s, int[] curseur, int fin) {
        while (curseur[0] < fin && (Character.isWhitespace(s.charAt(curseur[0])) || s.charAt(curseur[0]) == ',')) {
            curseur[0]++;
        }
    }
