...
public class ServiceValidation {
            ...
            .orElseThrow(
                () ->
                    new RegleMetierException(
                        "Aucune session d'enregistrement pour le passage "
                            + idPassage
  ...
  public StatutObservation statut(Observation observation) {
    ...
    if (observation.taxonObservateur().equals(observation.taxonTadarida())) {
      return observation.probObservateur() != null
  ...
  public List<LigneObservation> lignesAExporter(Long idResultats) {
    ...
    for (Observation o : observationDao.findByResults(idResultats)) {
      String nom =
          nomParSequence.computeIfAbsent(
              o.idSequence(),
              id ->
                  sequenceDao
                      .findById(id)
                      ...
                      .orElseThrow(
                          () ->
                              new RegleMetierException(
                                  "Séquence " + id + " introuvable pour l'export.")));
      lignes.add(
