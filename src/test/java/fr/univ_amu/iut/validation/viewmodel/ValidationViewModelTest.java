...
class ValidationViewModelTest {
    ...
    private static final Taxon NOCTULE = new Taxon("NYCNOC", "Nyctalus noctula", "Noctule commune", 1L);
    ...
    void ouvrir_sans_resultats() {
        ...
        assertThat(viewModel.messageProperty().get()).isEqualTo("Aucun résultat Tadarida importé pour ce passage.");
    }

    @Test
    @DisplayName("ouvrirSur : un CSV importé sans détection est distingué de l'absence d'import")
    void ouvrir_csv_sans_detection() {
        when(service.chargerValidation(ID_PASSAGE)).thenReturn(new VueValidation(ID_RESULTATS, List.of()));
    ...
    void ouvrir_en_erreur() {
        ...
        when(service.chargerValidation(ID_PASSAGE)).thenThrow(new RegleMetierException("Passage introuvable : 42"));
        viewModel.ouvrirSur(ID_PASSAGE);
    ...
    void valider_applique_et_recharge() {
        VueValidation apres = new VueValidation(
                ID_RESULTATS,
                List.of(
    ...
    void exporter_en_erreur_restitue_le_message() {
        Path dest = Path.of("x_Vu.csv");
        when(service.chargerValidation(ID_PASSAGE)).thenReturn(vueTrois());
        when(service.exporter(ID_RESULTATS, dest, true)).thenThrow(new RuntimeException("Disque plein"));
        viewModel.ouvrirSur(ID_PASSAGE);
    ...
    void importer_charge_les_resultats() {
        ...
        when(service.chargerValidation(ID_PASSAGE)).thenReturn(new VueValidation(null, List.of()), vueTrois());
        viewModel.ouvrirSur(ID_PASSAGE);
        assertThat(viewModel.observations()).isEmpty();
    ...
    void importer_en_erreur_restitue_le_message() {
        Path csv = Path.of("nuit-observations.csv");
        when(service.chargerValidation(ID_PASSAGE)).thenReturn(new VueValidation(null, List.of()));
        when(service.importer(ID_PASSAGE, csv)).thenThrow(new RegleMetierException("Aucune session d'enregistrement"));
        viewModel.ouvrirSur(ID_PASSAGE);
