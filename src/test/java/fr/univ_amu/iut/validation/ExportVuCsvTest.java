...
class ExportVuCsvTest {
  ...
  private static LigneObservation nonTouchee() {
    return new LigneObservation(
        "seq_000", 0.3, 3.9, 153, "noise", 0.93, null, null, null, ModeValidation.NON_VALIDE);
  }

  private static LigneObservation validee() {
    return new LigneObservation(
        "seq_001", 0.4, 4.8, 45, "Pippip", 0.8, null, "Pippip", 0.8, ModeValidation.MANUEL);
  }

  private static LigneObservation corrigee() {
    return new LigneObservation(
        "seq_002", 1.0, 2.0, 30, "Nyclei", 0.5, "Pippip", "Tadten", 0.9, ModeValidation.MANUEL);
  }
  ...
  @Test
  @DisplayName(
      "R17 : une ligne non touchée conserve ses colonnes Tadarida et laisse l'observateur vide")
  ...
  @Test
  @DisplayName(
      "Une ligne corrigée reflète un taxon observateur différent (R16) et garde la 2e propal Tadarida")
  void ligne_corrigee_reflete_observateur_different() {
    List<String> ligne = lecteur.lire(export.versChaine(List.of(corrigee()))).get(1);
  ...
  void r24_colonne_validation_mode() {
    LigneObservation auto =
        new LigneObservation(
            "seq_003", 0.0, 5.0, 60, "Pippip", 0.7, null, "Pippip", 0.7, ModeValidation.AUTO);

    List<List<String>> lignes =
        lecteur.lire(export.versChaine(List.of(nonTouchee(), validee(), auto), true));
  ...
  void sans_mode_pas_de_12e_colonne() {
    assertThat(lecteur.lire(export.versChaine(List.of(nonTouchee()))).get(0)).hasSize(11);
  }
}
