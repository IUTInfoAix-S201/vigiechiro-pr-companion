 ...
 * <b>optionnel</b> : un champ {@code null} signifie « ne pas filtrer sur ce critère ». Les critères
 * renseignés se combinent en ET logique (le passage doit satisfaire <b>tous</b> les critères
 * présents).
 *
 * <p>C'est aussi ce que mémorise une vue sauvegardée (« ⭐ Mes vues », story E5.S3) : {@link
 * ServiceMultisite#enregistrerVue(String, FiltresMultisite)} le sérialise en JSON ({@link
 * FiltresMultisiteJson}) dans la colonne {@code saved_view.filters_json}, et {@link
 * ServiceMultisite#chargerVue(Long)} le reconstruit.
 *
 * @param numeroCarre n° de carré du site à conserver, ou {@code null} pour tous les sites
 * @param statut statut de workflow à conserver, ou {@code null} pour tous les statuts
 * @param verdict verdict de vérification à conserver, ou {@code null} pour tous les verdicts
 * @param annee année à conserver, ou {@code null} pour toutes les années
 */
public record FiltresMultisite(
  ...
  /**
   * Indique si {@code ligne} satisfait <b>tous</b> les critères renseignés (les critères {@code
   * null} sont ignorés). C'est le prédicat appliqué par {@link ServiceMultisite} pour filtrer la
   * vue agrégée.
   */
