...
| **Lot prêt à déposer** | Dossier contenant toutes les séquences d'écoute d'une capture, formaté selon les attentes du portail Vigie-Chiro. | Sous-dossier `transformes/` de la capture (cf. [R22](Règles%20métier.md#r22)), à téléverser tel quel sur vigiechiro.herokuapp.com. |
| **Workspace** | Dossier racine sur le disque local de l'utilisateur où vivent toutes les captures, la base SQLite et les réglages. Configurable depuis les préférences (cf. [R21](Règles%20métier.md#r21)). | Défaut : `<Documents>/VigieChiro-Companion/`. Pour gros volumes, pointer un disque externe. |
