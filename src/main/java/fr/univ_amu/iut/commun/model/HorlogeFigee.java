...
/// Implémentation déterministe de [Horloge] : le temps est **figé** sur un instant fourni à
/// la construction. C'est l'équivalent maison de `java.time.Clock.fixed(...)`.
///
/// Réservée aux tests : on injecte cette horloge au service pour que toute écriture de date soit
/// prévisible (ex. asserter que la date de création d'un site vaut bien `"2026-05-31"`). Elle
/// vit en `commun.model` (et non dans `src/test`) pour être réutilisable par les tests
