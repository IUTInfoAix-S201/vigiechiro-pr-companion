...
/// Analyseur d'arguments **minimal et manuel** de la ligne de commande, sans aucune dépendance
/// externe (pour ne pas toucher au `pom.xml` ni au `module-info`). Il reconnaît des
/// options longues de la forme `--cle valeur` ; une option suivie d'une autre option (ou en
...
/// ```
///
/// Classe `model` pure (aucun import JavaFX) : elle ne fait que transformer un `String[]` en
/// table d'options consultable. Les valeurs manquantes ou mal typées lèvent une [ErreurUsage], que
