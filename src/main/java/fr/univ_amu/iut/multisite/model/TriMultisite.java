 ...
 * passage, statut, verdict et date »). Chaque valeur expose un {@link Comparator} déterministe via
 * {@link #comparateur()}.
 *
 * <p>Tous les comparateurs <b>brisent les égalités</b> avec le même ordre de référence (site,
 * point, année, n° de passage) pour garantir un résultat <b>stable</b> : deux exécutions sur les
 * mêmes données produisent exactement la même séquence de lignes (indispensable pour les exports
 * CSV « golden »). Le verdict, qui peut être {@code null} (passage non encore vérifié), est placé
 * en <b>fin</b> de tri.
 */
public enum TriMultisite {
  ...
  /** Tri par statut de workflow (ordre de progression {@link StatutWorkflow}), puis référence. */
  PAR_STATUT(Comparator.comparingInt((LignePassage ligne) -> ligne.statut().ordinal())),

  /** Tri par verdict ({@link Verdict}, passages non vérifiés en dernier), puis référence. */
  PAR_VERDICT(
      Comparator.comparing(
          LignePassage::verdict, Comparator.nullsLast(Comparator.comparingInt(Verdict::ordinal))));

  /** Ordre de référence stable, utilisé tel quel par {@link #PAR_SITE} et en départage ailleurs. */
   ...
   * référence). Construit à l'appel : {@link #REFERENCE} est alors initialisé (contrairement à
