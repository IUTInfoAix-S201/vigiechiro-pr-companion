 ...
 *
 * <p>Toute la résolution de chemins passe par cette classe via {@link Path} : <b>aucun chemin n'est
 * codé en dur</b> ailleurs (portabilité O1, accessibilité multi-OS). Le défaut est {@code
 * <Documents>/VigieChiro-Companion/} (R21), surchargé en test par un {@code @TempDir}.
 *
 * <pre>
 * &lt;workspace&gt;/
...
public final class Workspace {
  ...
  /**
   * Workspace par défaut : {@code <home>/Documents/VigieChiro-Companion} (R21). Le dossier n'est
   * pas créé ici : il le sera paresseusement à la première connexion (cf. {@code SourceDeDonnees}).
   */
  ...
  /** Sous-dossier {@code bruts/} d'une session (originaux R7). */
  public Path dossierBruts(String prefixe) {
    return dossierSession(prefixe).resolve(SOUS_DOSSIER_BRUTS);
  }

  /** Sous-dossier {@code transformes/} d'une session (séquences R8 + CSV R23). */
