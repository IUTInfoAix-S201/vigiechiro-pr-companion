...
public final class AnalyseAnomalies {
    ...
    public List<String> reveilsNonProgrammes() {
        return filtrer(
                a -> a.contains("réveil non programmé") || a.contains("reveil non programme") || a.contains("wakeup"));
    }
    ...
    public List<String> redemarrages() {
        return filtrer(
                a -> a.contains("redémarr") || a.contains("redemarr") || a.contains("reboot") || a.contains("restart"));
    }
    ...
    private List<String> filtrer(Predicate<String> surMinuscule) {
        return anomalies.stream()
                .filter(a -> surMinuscule.test(a.toLowerCase(Locale.ROOT)))
                .toList();
    }
}
