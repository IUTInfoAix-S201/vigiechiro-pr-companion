...
public final class Cli {
  ...
  private int importer(ArgumentsCli arguments, PrintStream sortie) {
    ...
    PointDEcoute point =
        pointDao
            .findById(idPoint)
            .orElseThrow(
                () -> new ErreurUsage("Point d'écoute introuvable : --point " + idPoint + "."));
    Site site =
        siteDao
  ...
  private int exporterLot(ArgumentsCli arguments, PrintStream sortie) {
    ...
    sortie.println(
        "  Volume    : "
            + (lot.volumeSequencesOctets() == null
                ? "-"
                : lot.volumeSequencesOctets() + " octets"));
  ...
  private int exporterVu(ArgumentsCli arguments, PrintStream sortie) {
            ...
            .orElseThrow(
                () ->
                    new RegleMetierException(
                        "Aucun résultat Tadarida importé pour le passage "
                            + idPassage
    ...
    Path ecrit =
        injecteur
            .getInstance(ServiceValidation.class)
            .exporter(resultats.id(), sortieFichier, true);
  ...
  private static int prochainNumero(PassageDao passageDao, long idPoint) {
    return passageDao.findByPoint(idPoint).stream().mapToInt(Passage::numeroPassage).max().orElse(0)
        + 1;
  }
